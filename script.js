...
const app = {
    ...
    init() {
        // --- Element Querying ---
        const ids = [
            'app', 'setup-screen', 'chat-screen', 'lang-ko', 'lang-en', 'theme-toggle',
            ...
            'message-area', 'typing-indicator', 'message-input', 'send-btn'
        ];
    ...
    addEventListeners() {
        ...
        if (!el.langKoBtn || !el.startChatBtn) {
            console.error("Critical elements not found. Clicks will not work.");
            alert("Error: UI elements could not be loaded. Please refresh the page.");
            return;
        }

        el.langKoBtn.addEventListener('click', () => this.setLanguage('ko'));
        el.langEnBtn.addEventListener('click', () => this.setLanguage('en'));
        el.getApiKeyBtn.addEventListener('click', () => { el.apiKeyModal.style.display = 'flex'; });
        el.closeModalBtn.addEventListener('click', () => { el.apiKeyModal.style.display = 'none'; });
        ...
        el.startChatBtn.addEventListener('click', () => this.startChat());
        el.backToSetupBtn.addEventListener('click', () => {
            this.showSetupScreen();
            clearTimeout(this.state.nudgeTimer);
        });
        el.resetSettingsBtn.addEventListener('click', () => this.resetSettings());
    ...
    applyTheme() {
        ...
        this.elements.themeToggle.textContent = this.state.lightTheme ? '🌙' : '☀️';
    },
    ...
    saveSettings() {
        ...
        localStorage.setItem('aiLoverSettings', JSON.stringify(settingsToSave));
    },

    loadSettings() {
        const savedSettings = JSON.parse(localStorage.getItem('aiLoverSettings'));
        if (savedSettings) {
            Object.assign(this.state, savedSettings);
        }
        this.elements.apiKeyInput.value = this.state.apiKey || '';
        if (this.state.persona) {
            this.elements.personaNameInput.value = this.state.persona.name || '';
            this.elements.personaRelationshipInput.value = this.state.persona.relationship || '';
            this.elements.personaMbtiInput.value = this.state.persona.mbti || '';
            this.elements.personaToneInput.value = this.state.persona.tone || '';
        }
    },
    ...
    async startChat() {
        ...
        if (typeof GoogleGenerativeAI === 'undefined') {
            alert('Could not connect to Google AI. Please check your network connection and refresh.');
            return;
        }
        ...
        if (timeSinceLast > (60 * 60 * 1000) && this.state.chatHistory.length > 0) {
    ...
    removeReadReceipts() {
        this.elements.messageArea.querySelectorAll('.read-receipt').forEach(el => el.remove());
    },

    resetNudgeTimer() {
        clearTimeout(this.state.nudgeTimer);
        const randomInterval = (Math.random() * 4 * 60 * 1000) + (1 * 60 * 1000);
    ...
    async sendNudgeMessage() {
        if (this.isLastMessageFromUser() && !this.state.isTyping && this.state.chat) {
            const nudgePrompt = "I haven't heard from you in a bit, say something to gently restart the conversation based on your persona.";
    ...
    async sendWelcomeBackMessage() {
        if (!this.state.isTyping && this.state.chat) {
            const welcomePrompt = "I'm opening the app again after a long time. Greet me in character, like you missed me.";
    ...
    resetSettings() {
        if (confirm('Are you sure you want to reset all settings? This will clear your chat history.')) {
            ...
            Object.assign(this.state, {
                apiKey: null, language: 'ko', accentTheme: 'theme-yellow', lightTheme: true,
                persona: { name: '', relationship: '', mbti: '', tone: '' },
                chat: null, chatHistory: [], isTyping: false, nudgeTimer: null, lastMessageTimestamp: null,
            });
            this.loadSettings();
            this.applyTheme();
            this.updateUI();
